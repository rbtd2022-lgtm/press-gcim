...
(function () {
  ...
  async function loadPublication() {
    ...
    if (window.GCIM_PUBLICATION_DATA) {
      const item = window.GCIM_PUBLICATION_DATA;
      if (
        ((!id || item.id === id) && (!slug || item.slug === slug)) &&
    ...
    if (window.GCIM_NEWS_DATA) {
      const items = Array.isArray(window.GCIM_NEWS_DATA.publications)
        ? window.GCIM_NEWS_DATA.publications
        : [];
  ...
  function updateUrl(lang) {
    ...
    history.replaceState(null, '', url.pathname.split('/').pop() + '?' + url.searchParams.toString() + url.hash);
  }
  ...
  function render(lang) {
    ...
    if (brand) brand.href = 'index.html?lang=' + encodeURIComponent(lang);
    const backToNews = document.getElementById('backToNews');
    if (backToNews) backToNews.href = 'index.html?lang=' + encodeURIComponent(lang);
  ...
  function wireUi() {
    ...
    const brand = document.querySelector('a.brand');
    if (brand) {
      brand.href = 'index.html?lang=' + encodeURIComponent(currentLang);
      brand.onclick = (event) => {
        event.preventDefault();
        window.location.href = 'index.html?lang=' + encodeURIComponent(currentLang);
      };
    }
