...
STATIC_SECTIONS = ("about", "media-inquiries")
...
TYPOGRAPHY_START = "/* GCIM_GENERATED_LATIN_NEWS_TYPOGRAPHY_START */"
TYPOGRAPHY_END = "/* GCIM_GENERATED_LATIN_NEWS_TYPOGRAPHY_END */"
PUBLICATION_TYPOGRAPHY_START = "/* GCIM_GENERATED_LATIN_PUBLICATION_TYPOGRAPHY_START */"
PUBLICATION_TYPOGRAPHY_END = "/* GCIM_GENERATED_LATIN_PUBLICATION_TYPOGRAPHY_END */"
SECTION_TYPOGRAPHY_START = "/* GCIM_GENERATED_LATIN_SECTION_TYPOGRAPHY_START */"
SECTION_TYPOGRAPHY_END = "/* GCIM_GENERATED_LATIN_SECTION_TYPOGRAPHY_END */"
CYRILLIC_LISTING_TYPOGRAPHY_START = "/* GCIM_GENERATED_CYRILLIC_NEWS_TYPOGRAPHY_START */"
CYRILLIC_LISTING_TYPOGRAPHY_END = "/* GCIM_GENERATED_CYRILLIC_NEWS_TYPOGRAPHY_END */"
CYRILLIC_PUBLICATION_TYPOGRAPHY_START = "/* GCIM_GENERATED_CYRILLIC_PUBLICATION_TYPOGRAPHY_START */"
CYRILLIC_PUBLICATION_TYPOGRAPHY_END = "/* GCIM_GENERATED_CYRILLIC_PUBLICATION_TYPOGRAPHY_END */"
CYRILLIC_SECTION_TYPOGRAPHY_START = "/* GCIM_GENERATED_CYRILLIC_SECTION_TYPOGRAPHY_START */"
CYRILLIC_SECTION_TYPOGRAPHY_END = "/* GCIM_GENERATED_CYRILLIC_SECTION_TYPOGRAPHY_END */"
PORTAL_RESOURCES_START = "/* GCIM_GENERATED_PORTAL_RESOURCES_START */"
PORTAL_RESOURCES_END = "/* GCIM_GENERATED_PORTAL_RESOURCES_END */"
LEGACY_HOME_TYPOGRAPHY = re.compile(r'\s*/\* Desktop English news list: measured from whitehouse\.gov/news/ \*/.*?(?=\s*</style>)', re.S)
...
def portal_resources_markup(lang, portal):
    """Render the one desktop information line and preserve the short mobile line."""
    mobile_title = escape(portal["mobileTitle"][lang])
    desktop = escape(portal["desktop"][lang])
    links = " · ".join(f'<a href="{escape(url, quote=True)}">{escape(url)}</a>' for url in portal["urls"])
    mobile_lines = "".join(
        '<span class="portal-bar-mobile-links">' + " · ".join(
            f'<a href="{escape(url, quote=True)}">{escape(url.removeprefix("https://").rstrip("/"))}</a>' for url in group
        ) + '</span>'
        for group in (portal["urls"][:2], portal["urls"][2:])
    )
    return (
        '<div class="portal-bar-inner">'
        f'<span class="portal-bar-mobile"><span class="portal-bar-mobile-title">{mobile_title}</span>{mobile_lines}</span>'
        f'<span class="portal-bar-desktop">{desktop} {links}</span>'
        '</div>'
    )


def apply_portal_resources(root):
    """Apply the approved desktop resources line to every page that has a portal bar."""
    config_path = root / "content/portal-resources.json"
    css_path = root / "content/portal-resources.css"
    require(config_path.is_file() and css_path.is_file(), "Missing portal resources templates")
    portal = json.loads(config_path.read_text(encoding="utf-8"))
    require(set(portal.get("mobileTitle", {})) == set(LANGS), "Portal mobile languages are incomplete")
    require(set(portal.get("desktop", {})) == set(LANGS), "Portal desktop languages are incomplete")
    urls = portal.get("urls", [])
    require(urls == ["https://gcim.eu/", "https://press.gcim.eu/", "https://news.gcim.eu/", "https://pressroom.gcim.eu/"], "Portal URL list changed")
    css = css_path.read_text(encoding="utf-8").strip()
    require('assets/fonts/instrument-sans-whitehouse-regular.ttf' in css, "Portal Latin font must remain local")
    require('"Libre Caslon Cyrillic Condensed"' in css, "Portal Cyrillic font changed")
    markup_pattern = re.compile(r'<div\b[^>]*\bclass="portal-bar-inner"[^>]*>.*?</div>', re.S)
    css_pattern = re.compile(re.escape(PORTAL_RESOURCES_START) + r".*?" + re.escape(PORTAL_RESOURCES_END), re.S)
    generated = f"\n    {PORTAL_RESOURCES_START}\n" + "\n".join("    " + line if line else "" for line in css.splitlines()) + f"\n    {PORTAL_RESOURCES_END}\n"
    changed = 0
    for path in root.glob("*.html"):
        source = path.read_text(encoding="utf-8")
        if not markup_pattern.search(source):
            continue
        lang_match = re.search(r'<html\b[^>]*\blang="([a-z]+)"', source, re.I)
        require(lang_match is not None and lang_match.group(1).lower() in LANGS, f"Missing page language: {path.name}")
        lang = lang_match.group(1).lower()
        source, count = markup_pattern.subn(portal_resources_markup(lang, portal), source, count=1)
        require(count == 1, f"Missing portal resources line: {path.name}")
        if css_pattern.search(source):
            source = css_pattern.sub(generated.strip(), source)
        else:
            position = source.rfind("</style>")
            require(position != -1, f"Missing stylesheet: {path.name}")
            source = source[:position] + generated + source[position:]
        path.write_text(source, encoding="utf-8")
        changed += 1
    require(changed > 0, "No portal resources lines updated")


def validate_portal_resources(root):
    urls = ("https://gcim.eu/", "https://press.gcim.eu/", "https://news.gcim.eu/", "https://pressroom.gcim.eu/")
    checked = 0
    for path in root.glob("*.html"):
        source = path.read_text(encoding="utf-8")
        if 'class="portal-bar-mobile"' not in source:
            continue
        require(source.count('class="portal-bar-mobile"') == 1, f"Invalid mobile portal line: {path.name}")
        require(source.count('class="portal-bar-desktop"') == 1, f"Invalid desktop portal line: {path.name}")
        require(all(f'href="{url}"' in source for url in urls), f"Missing portal URL: {path.name}")
        require(source.count(PORTAL_RESOURCES_START) == source.count(PORTAL_RESOURCES_END) == 1, f"Missing portal CSS: {path.name}")
        css = source[source.index(PORTAL_RESOURCES_START):source.index(PORTAL_RESOURCES_END)]
        require('@media (min-width:841px)' in css and '@media (max-width:840px)' in css, f"Portal CSS not desktop-only: {path.name}")
        checked += 1
    require(checked > 0, "No portal pages checked")


def apply_copyright(root):
    """Keep one approved copyright line on every screen and future generated page."""
    block_pattern = re.compile(r'<div class="copyright">(.*?)</div>', re.S)
    def replacement(match):
        return '<div class="copyright">© 2020-2027 Global Citizens. All Rights Reserved</div>'
    changed = 0
    for path in root.glob("*.html"):
        source = path.read_text(encoding="utf-8")
        if '<div class="copyright">' not in source:
            continue
        source, count = block_pattern.subn(replacement, source, count=1)
        require(count == 1, f"Missing copyright block: {path.name}")
        path.write_text(source, encoding="utf-8")
        changed += 1
    require(changed > 0, "No copyright blocks updated")


def validate_copyright(root):
    expected = "© 2020-2027 Global Citizens. All Rights Reserved"
    checked = 0
    for path in root.glob("*.html"):
        source = path.read_text(encoding="utf-8")
        if '<div class="copyright">' not in source:
            continue
        require(source.count('<div class="copyright">') == 1, f"Invalid copyright block: {path.name}")
        require(f'<div class="copyright">{expected}</div>' in source, f"Incorrect copyright: {path.name}")
        checked += 1
    require(checked > 0, "No copyright blocks checked")
...
def apply_listing_typography(root):
    """Inject the maintained desktop typography template into every listing."""
    template_path = root / "content/news-listing-typography.css"
    require(template_path.is_file(), "Missing news listing typography template")
    template = template_path.read_text(encoding="utf-8").strip()
    require('assets/fonts/instrument-serif-whitehouse-regular.woff2' in template, "Typography template must use the local Instrument Serif file")
    require('font-size:clamp(1.5rem, 0.4vw + 1.39rem, 1.75rem);' in template, "Typography template font size changed")
    require('line-height:1.18;' in template and 'letter-spacing:.56px;' in template, "Typography template metrics changed")
    for stem in LISTINGS:
        for lang in ("en", "fr", "es"):
            name = stem + ("" if lang == "en" else f"-{lang}") + ".html"
            path = root / name
            source = path.read_text(encoding="utf-8")
            source = LEGACY_HOME_TYPOGRAPHY.sub("\n", source)
            css = template
            generated = f"\n    {TYPOGRAPHY_START}\n" + "\n".join("    " + line if line else "" for line in css.splitlines()) + f"\n    {TYPOGRAPHY_END}\n"
            pattern = re.compile(re.escape(TYPOGRAPHY_START) + r".*?" + re.escape(TYPOGRAPHY_END), re.S)
            if pattern.search(source):
                source = pattern.sub(generated.strip(), source)
            else:
                position = source.rfind("</style>")
                require(position != -1, f"Missing stylesheet: {name}")
                source = source[:position] + generated + source[position:]
            path.write_text(source, encoding="utf-8")


def validate_listing_typography(root):
    """Keep the future-news typography rule tied to the generator, not hand edits."""
    required = (
        'assets/fonts/instrument-serif-whitehouse-regular.woff2',
        'font-size:clamp(1.5rem, 0.4vw + 1.39rem, 1.75rem);',
        'font-weight:400;',
        'font-style:normal;',
        'line-height:1.18;',
        'letter-spacing:.56px;',
        'max-width:min(100%, 782px);',
        'margin-top:clamp(1.25rem, 1vw + 0.975rem, 1.875rem);',
    )
    for stem in LISTINGS:
        for lang in LANGS:
            name = stem + ("" if lang == "en" else f"-{lang}") + ".html"
            source = (root / name).read_text(encoding="utf-8")
            has_generated_rule = TYPOGRAPHY_START in source
            if lang in ("en", "fr", "es"):
                require(has_generated_rule and source.count(TYPOGRAPHY_START) == source.count(TYPOGRAPHY_END) == 1, f"Missing generated typography: {name}")
                rule = source[source.index(TYPOGRAPHY_START):source.index(TYPOGRAPHY_END)]
                require(all(value in rule for value in required), f"Incorrect generated typography: {name}")
                require('.featured h2{' in rule, f"Missing generated featured typography: {name}")
            else:
                require(not has_generated_rule, f"Latin typography applied to non-Latin page: {name}")


def apply_cyrillic_listing_typography(root):
    """Inject maintained desktop typography into Russian and Ukrainian listings."""
    template_path = root / "content/cyrillic-news-listing-typography.css"
    require(template_path.is_file(), "Missing Cyrillic news listing typography template")
    template = template_path.read_text(encoding="utf-8").strip()
    required = (
        'font-family:"Libre Caslon Cyrillic Condensed",Georgia,"Times New Roman",serif;',
        'font-size:clamp(1.5rem, 0.4vw + 1.39rem, 1.75rem);',
        'line-height:1.18;', 'letter-spacing:.56px;', 'max-width:min(100%, 782px);',
    )
    require(all(value in template for value in required), "Cyrillic listing typography template changed")
    pattern = re.compile(re.escape(CYRILLIC_LISTING_TYPOGRAPHY_START) + r".*?" + re.escape(CYRILLIC_LISTING_TYPOGRAPHY_END), re.S)
    generated = f"\n    {CYRILLIC_LISTING_TYPOGRAPHY_START}\n" + "\n".join("    " + line if line else "" for line in template.splitlines()) + f"\n    {CYRILLIC_LISTING_TYPOGRAPHY_END}\n"
    for stem in LISTINGS:
        for lang in ("ru", "uk"):
            name = stem + f"-{lang}.html"
            path = root / name
            source = path.read_text(encoding="utf-8")
            if pattern.search(source):
                source = pattern.sub(generated.strip(), source)
...
def validate_cyrillic_listing_typography(root):
    required = (
        'font-family:"Libre Caslon Cyrillic Condensed",Georgia,"Times New Roman",serif;',
        'font-size:clamp(1.5rem, 0.4vw + 1.39rem, 1.75rem);',
        'font-weight:400;', 'font-style:normal;', 'line-height:1.18;', 'letter-spacing:.56px;',
        'max-width:min(100%, 782px);', '.featured h2{',
    )
    for stem in LISTINGS:
        for lang in LANGS:
            name = stem + ("" if lang == "en" else f"-{lang}") + ".html"
            source = (root / name).read_text(encoding="utf-8")
            present = CYRILLIC_LISTING_TYPOGRAPHY_START in source
            if lang in ("ru", "uk"):
                require(present and source.count(CYRILLIC_LISTING_TYPOGRAPHY_START) == source.count(CYRILLIC_LISTING_TYPOGRAPHY_END) == 1, f"Missing Cyrillic listing typography: {name}")
                rule = source[source.index(CYRILLIC_LISTING_TYPOGRAPHY_START):source.index(CYRILLIC_LISTING_TYPOGRAPHY_END)]
                require(all(value in rule for value in required), f"Incorrect Cyrillic listing typography: {name}")
            else:
                require(not present, f"Cyrillic typography applied to non-Cyrillic page: {name}")


def apply_publication_typography(root, news):
    """Inject the maintained desktop White House typography into Latin publications."""
    template_path = root / "content/publication-typography.css"
    require(template_path.is_file(), "Missing publication typography template")
    template = template_path.read_text(encoding="utf-8").strip()
    required = (
        'assets/fonts/instrument-serif-whitehouse-regular.ttf',
        'assets/fonts/instrument-sans-whitehouse-regular.ttf',
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);',
        'font-size:clamp(1rem, 0.2vw + 0.945rem, 1.125rem);',
        'line-height:1.08;', 'line-height:1.6;', 'letter-spacing:1.2px;', 'letter-spacing:.2px;',
    )
    require(all(value in template for value in required), "Publication typography template changed")
    pattern = re.compile(re.escape(PUBLICATION_TYPOGRAPHY_START) + r".*?" + re.escape(PUBLICATION_TYPOGRAPHY_END), re.S)
    generated = f"\n    {PUBLICATION_TYPOGRAPHY_START}\n" + "\n".join("    " + line if line else "" for line in template.splitlines()) + f"\n    {PUBLICATION_TYPOGRAPHY_END}\n"
    for item in news["publications"]:
        for lang in ("en", "fr", "es"):
            path = root / f"{item['id']}-{lang}.html"
            source = path.read_text(encoding="utf-8")
            if pattern.search(source):
                source = pattern.sub(generated.strip(), source)
            else:
...
def validate_publication_typography(root, news):
    required = (
        ...
        'max-width:min(100%, 1080px);', 'max-width:min(100%, 782px);',
    )
    for item in news["publications"]:
        for lang in LANGS:
            path = root / f"{item['id']}-{lang}.html"
            source = path.read_text(encoding="utf-8")
            has_generated_rule = PUBLICATION_TYPOGRAPHY_START in source
            if lang in ("en", "fr", "es"):
                require(has_generated_rule and source.count(PUBLICATION_TYPOGRAPHY_START) == source.count(PUBLICATION_TYPOGRAPHY_END) == 1, f"Missing generated publication typography: {path.name}")
                rule = source[source.index(PUBLICATION_TYPOGRAPHY_START):source.index(PUBLICATION_TYPOGRAPHY_END)]
                require(all(value in rule for value in required), f"Incorrect generated publication typography: {path.name}")
            else:
                require(not has_generated_rule, f"Latin publication typography applied to non-Latin page: {path.name}")


def apply_cyrillic_publication_typography(root, news):
    """Inject maintained desktop typography into Russian and Ukrainian publications."""
    template_path = root / "content/cyrillic-publication-typography.css"
    require(template_path.is_file(), "Missing Cyrillic publication typography template")
    template = template_path.read_text(encoding="utf-8").strip()
    required = (
        'font-family:"Libre Caslon Cyrillic Condensed",Georgia,"Times New Roman",serif;',
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);',
    ...
    require(all(value in template for value in required), "Cyrillic publication typography template changed")
    pattern = re.compile(re.escape(CYRILLIC_PUBLICATION_TYPOGRAPHY_START) + r".*?" + re.escape(CYRILLIC_PUBLICATION_TYPOGRAPHY_END), re.S)
    generated = f"\n    {CYRILLIC_PUBLICATION_TYPOGRAPHY_START}\n" + "\n".join("    " + line if line else "" for line in template.splitlines()) + f"\n    {CYRILLIC_PUBLICATION_TYPOGRAPHY_END}\n"
    for item in news["publications"]:
        for lang in ("ru", "uk"):
            path = root / f"{item['id']}-{lang}.html"
            source = path.read_text(encoding="utf-8")
...
def validate_cyrillic_publication_typography(root, news):
    required = (
        'font-family:"Libre Caslon Cyrillic Condensed",Georgia,"Times New Roman",serif;',
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);', 'line-height:1.08;', 'letter-spacing:1.2px;',
        'font-size:clamp(1rem, 0.2vw + 0.945rem, 1.125rem);', 'line-height:1.6;', 'letter-spacing:.2px;',
        'max-width:min(100%, 1080px);', 'max-width:min(100%, 782px);',
    )
    for item in news["publications"]:
        for lang in LANGS:
            path = root / f"{item['id']}-{lang}.html"
            source = path.read_text(encoding="utf-8")
            present = CYRILLIC_PUBLICATION_TYPOGRAPHY_START in source
            if lang in ("ru", "uk"):
                require(present and source.count(CYRILLIC_PUBLICATION_TYPOGRAPHY_START) == source.count(CYRILLIC_PUBLICATION_TYPOGRAPHY_END) == 1, f"Missing Cyrillic publication typography: {path.name}")
                rule = source[source.index(CYRILLIC_PUBLICATION_TYPOGRAPHY_START):source.index(CYRILLIC_PUBLICATION_TYPOGRAPHY_END)]
                require(all(value in rule for value in required), f"Incorrect Cyrillic publication typography: {path.name}")
            else:
                require(not present, f"Cyrillic publication typography applied to non-Cyrillic page: {path.name}")


def apply_static_section_typography(root):
    """Apply the same desktop title/body typography to the approved full sections."""
    template_path = root / "content/static-section-typography.css"
    require(template_path.is_file(), "Missing static section typography template")
    template = template_path.read_text(encoding="utf-8").strip()
    required = (
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);',
        'font-size:clamp(1rem, 0.2vw + 0.945rem, 1.125rem);',
    ...
    require(all(value in template for value in required), "Static section typography template changed")
    pattern = re.compile(re.escape(SECTION_TYPOGRAPHY_START) + r".*?" + re.escape(SECTION_TYPOGRAPHY_END), re.S)
    generated = f"\n    {SECTION_TYPOGRAPHY_START}\n" + "\n".join("    " + line if line else "" for line in template.splitlines()) + f"\n    {SECTION_TYPOGRAPHY_END}\n"
    for stem in STATIC_SECTIONS:
        for lang in ("en", "fr", "es"):
            path = root / (stem + ("" if lang == "en" else f"-{lang}") + ".html")
            source = path.read_text(encoding="utf-8")
            if pattern.search(source):
                source = pattern.sub(generated.strip(), source)
            else:
...
def validate_static_section_typography(root):
    required = (
        '.hero h1{', 'font-family:"Instrument Serif", serif;',
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);', 'line-height:1.08;', 'letter-spacing:1.2px;',
        'font-family:"Instrument Sans", sans-serif;',
        'font-size:clamp(1rem, 0.2vw + 0.945rem, 1.125rem);', 'line-height:1.6;', 'letter-spacing:.2px;',
    )
    for stem in STATIC_SECTIONS:
        for lang in LANGS:
            path = root / (stem + ("" if lang == "en" else f"-{lang}") + ".html")
            source = path.read_text(encoding="utf-8")
            has_generated_rule = SECTION_TYPOGRAPHY_START in source
            if lang in ("en", "fr", "es"):
                require(has_generated_rule and source.count(SECTION_TYPOGRAPHY_START) == source.count(SECTION_TYPOGRAPHY_END) == 1, f"Missing generated section typography: {path.name}")
                rule = source[source.index(SECTION_TYPOGRAPHY_START):source.index(SECTION_TYPOGRAPHY_END)]
                require(all(value in rule for value in required), f"Incorrect generated section typography: {path.name}")
            else:
                require(not has_generated_rule, f"Latin section typography applied to non-Latin page: {path.name}")


def apply_cyrillic_static_section_typography(root):
    """Apply desktop Cyrillic typography to the approved Russian and Ukrainian sections."""
    template_path = root / "content/cyrillic-static-section-typography.css"
    require(template_path.is_file(), "Missing Cyrillic static section typography template")
    template = template_path.read_text(encoding="utf-8").strip()
    required = (
        'font-family:"Libre Caslon Cyrillic Condensed",Georgia,"Times New Roman",serif;',
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);',
    ...
    require(all(value in template for value in required), "Cyrillic static section typography template changed")
    pattern = re.compile(re.escape(CYRILLIC_SECTION_TYPOGRAPHY_START) + r".*?" + re.escape(CYRILLIC_SECTION_TYPOGRAPHY_END), re.S)
    generated = f"\n    {CYRILLIC_SECTION_TYPOGRAPHY_START}\n" + "\n".join("    " + line if line else "" for line in template.splitlines()) + f"\n    {CYRILLIC_SECTION_TYPOGRAPHY_END}\n"
    for stem in STATIC_SECTIONS:
        for lang in ("ru", "uk"):
            path = root / f"{stem}-{lang}.html"
            source = path.read_text(encoding="utf-8")
            if pattern.search(source):
                source = pattern.sub(generated.strip(), source)
            else:
...
def validate_cyrillic_static_section_typography(root):
    required = (
        '.hero h1{', 'font-family:"Libre Caslon Cyrillic Condensed",Georgia,"Times New Roman",serif;',
        'font-size:clamp(3rem, 1.2vw + 2.67rem, 3.75rem);', 'line-height:1.08;', 'letter-spacing:1.2px;',
        'font-size:clamp(1rem, 0.2vw + 0.945rem, 1.125rem);', 'line-height:1.6;', 'letter-spacing:.2px;',
    )
    for stem in STATIC_SECTIONS:
        for lang in LANGS:
            path = root / (stem + ("" if lang == "en" else f"-{lang}") + ".html")
            source = path.read_text(encoding="utf-8")
            present = CYRILLIC_SECTION_TYPOGRAPHY_START in source
            if lang in ("ru", "uk"):
                require(present and source.count(CYRILLIC_SECTION_TYPOGRAPHY_START) == source.count(CYRILLIC_SECTION_TYPOGRAPHY_END) == 1, f"Missing Cyrillic section typography: {path.name}")
                rule = source[source.index(CYRILLIC_SECTION_TYPOGRAPHY_START):source.index(CYRILLIC_SECTION_TYPOGRAPHY_END)]
                require(all(value in rule for value in required), f"Incorrect Cyrillic section typography: {path.name}")
            else:
                require(not present, f"Cyrillic typography applied to non-Cyrillic section: {path.name}")
...
def build(source, output):
    ...
    with tempfile.TemporaryDirectory(prefix="press-office-build-") as folder:
        ...
        apply_listing_typography(stage)
        apply_cyrillic_listing_typography(stage)
        apply_publication_typography(stage, news)
        apply_cyrillic_publication_typography(stage, news)
        apply_static_section_typography(stage)
        apply_cyrillic_static_section_typography(stage)
        apply_portal_resources(stage)
        apply_copyright(stage)
        if new_ids:
        ...
        validate_listing_typography(stage)
        validate_cyrillic_listing_typography(stage)
        validate_publication_typography(stage, news)
        validate_cyrillic_publication_typography(stage, news)
        validate_static_section_typography(stage)
        validate_cyrillic_static_section_typography(stage)
        validate_portal_resources(stage)
        validate_copyright(stage)
